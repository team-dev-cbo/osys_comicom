...
//--------------------------------------------------------------------
//
//     GESTION DE L'AFFICHAGE DU CALENDRIER RESSOURCE
//
// Fonction  : callCalendrierEquipement
//              (Utilisation de la méthode AJAX)
//       a) Appel à la SERVLET
//--------------------------------------------------------------------
function callCalendrierEquipement(mode){

    if(verif_date(document.getElementById("idDateCaleRes"))) {

        document.body.style.cursor='wait';
        showObject("dataOnLoad");

        var atel   = document.getElementById("idListeAtelCaleRes").value;
        var geog   = document.getElementById("idListeSecteurCaleRes").value;
        var date   = document.getElementById("idDateCaleRes").value;
        var nbWeek = document.getElementById("idNbSemaineCaleRes").value;
        ...
        if(mode=="up"){
            ...
            date = dateAdd(date, 7);
            document.getElementById("idDateCaleRes").value = date;
        }
        if(mode=="doubleup"){
            //--> on ajoute 7 jours à la date saisie
        ...
        var handlerFunction = waitCalendrierEquipement; // on définie la méthode qui gérera la réponse de la requete Ajax
        reqCalRes.onreadystatechange = handlerFunction;
        reqCalRes.open("POST", contextPath+"/CalendrierEquipementMainServlet?atel="+encodeURIComponent(atel)+"&geog="+encodeURIComponent(geog)+"&date="+date+"&nbWeek="+nbWeek+"&width="+frameWidth+"&codeSite="+codeSite+"&affichage="+encodeURIComponent(affichage)+"&defaut="+encodeURIComponent(defaut), true);
...
// Fonction  : waitCalendrierEquipement
//              (Utilisation de la méthode AJAX)
...
function waitCalendrierEquipement() {
...
function openAddForm(){
    ...
    if(ListeSelection.length == 0){
        ...
        document.getElementById("idMachAjout1").checked = "checked";
...
function openDeleteForm(){
    ...
    if(ListeSelection.length == 0){
        ...
        document.getElementById("idMachDelete1").checked = "checked";
...
function openCalcForm(){
    ...
    if(ListeSelection.length == 0){
        ...
        document.getElementById("idMachCalcul1").checked = "checked";
...
//
//--------------------------------------------------------------------
// Fonction  : callCalendrierEquipementAdd
//       Ajout d'un nouvel Horaire dans le calendrier
//              (Utilisation de la méthode AJAX)
//       a) Appel à la SERVLET
//--------------------------------------------------------------------
function callCalendrierEquipementAdd(){
    ...
    var br       = document.getElementsByName("idMachAjout");
    var atel     = document.getElementById("idListeAtelCaleRes").value;
    var secteur  = document.getElementById("idListeSecteurCaleRes").value;
    var codeSite = document.getElementById("codeSite").value;
    ...
    var handlerFunction = waitCalendrierEquipementAdd; // on définie la méthode qui gérera la réponse de la requete Ajax
    req.onreadystatechange = handlerFunction;
    req.open("POST", contextPath+"/CalendrierEquipementAjoutServlet?atel="+encodeURIComponent(atel)+"&secteur="+encodeURIComponent(secteur)+"&codeSite="+encodeURIComponent(codeSite)+"&codeHora="+encodeURIComponent(codeHora)+"&dateDebu="+dateDebu+"&dateFin="+dateFin+"&deltaUTC="+deltaUTC+"&listeRes="+encodeURIComponent(listeRes), true);
    req.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
    req.send(null);
}
// Fonction  : waitCalendrierEquipementAdd
//              (Utilisation de la méthode AJAX)
//       b) Attente de la réponse de la SERVLET
//             Lorsque les 2 condition suivantes sont remplies (readyState==4 et status=200),
//             La fonction qui va traiter le résultat de la requête est appelée
function waitCalendrierEquipementAdd() {
...
// Fonction  : showCalendrierEquipementAdd
//              (Utilisation de la méthode AJAX)
//       c) Réponse de la SERVLET reçue
//              on recharge le calendrier après modification
function showCalendrierEquipementAdd() {

    // récupération de l'objet XML
    var xmldoc = req.responseXML;

    //--- récupération de la liste des balises <of>
    var xmlServlet = xmldoc.getElementsByTagName("insertom");
...
//--------------------------------------------------------------------
// Fonction  : callCalendrierEquipementDelete
//       Suppression d'un Horaire dans le calendrier
//              (Utilisation de la méthode AJAX)
//       a) Appel à la SERVLET
//--------------------------------------------------------------------
function callCalendrierEquipementDelete(){
    ...
    //--> Récupération des données saisies
    var br       = document.getElementsByName("idMachDelete");
    var codeSite = document.getElementById("codeSite").value;
    var deltaUTC = document.getElementById("deltaUTC").value;
    ...
    req = newXMLHttpRequest();
    var handlerFunction = waitCalendrierEquipementDelete; // on définie la méthode qui gérera la réponse de la requete Ajax
    req.onreadystatechange = handlerFunction;
    req.open("POST", contextPath+"/CalendrierEquipementSupprimeServlet?dateDebu="+dateDebu+"&dateFin="+dateFin+"&deltaUTC="+deltaUTC+"&codeSite="+encodeURIComponent(codeSite)+"&listeRes="+encodeURIComponent(listeRes)+"&type="+encodeURIComponent(type), true);
    req.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
    req.send(null);

}
// Fonction  : waitCalendrierEquipementDelete
//              (Utilisation de la méthode AJAX)
//       b) Attente de la réponse de la SERVLET
//             Lorsque les 2 condition suivantes sont remplies (readyState==4 et status=200),
//             La fonction qui va traiter le résultat de la requête est appelée
function waitCalendrierEquipementDelete() {
    ...
    if (req.readyState == 4) {
        ...
        if (req.status == 200) {
            ...
            showCalendrierEquipementDelete();
...
// Fonction  : showCalendrierEquipementDelete
//              (Utilisation de la méthode AJAX)
//       c) Réponse de la SERVLET reçue
//              on recharge le calendrier après modification
function showCalendrierEquipementDelete() {
...
//--------------------------------------------------------------------
// Fonction  : callCalendrierEquipementCalcul
//       Suppression d'un Horaire dans le calendrier
//              (Utilisation de la méthode AJAX)
//       a) Appel à la SERVLET
//--------------------------------------------------------------------
function callCalendrierEquipementCalcul(){
    ...
    var atel     = document.getElementById("idListeAtelCaleRes").value;
    var secteur  = document.getElementById("idListeSecteurCaleRes").value;
    var br       = document.getElementsByName("idMachCalcul");
    var codeSite = document.getElementById("codeSite").value;
    var deltaUTC = document.getElementById("deltaUTC").value;
    ...
    req = newXMLHttpRequest();
    var handlerFunction = waitCalendrierEquipementCalc; // on définie la méthode qui gérera la réponse de la requete Ajax
    req.onreadystatechange = handlerFunction;
    req.open("POST", contextPath+"/CalendrierEquipementCalculServlet?atel="+encodeURIComponent(atel)+"&secteur="+encodeURIComponent(secteur)+"&codeSite="+encodeURIComponent(codeSite)+"&dateDebu="+dateDebu+"&dateFin="+dateFin+"&deltaUTC="+deltaUTC+"&listeRes="+encodeURIComponent(listeRes), true);
    req.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
    req.send(null);

}
// Fonction  : waitCalendrierEquipementCalc
//              (Utilisation de la méthode AJAX)
//       b) Attente de la réponse de la SERVLET
//             Lorsque les 2 condition suivantes sont remplies (readyState==4 et status=200),
//             La fonction qui va traiter le résultat de la requête est appelée
function waitCalendrierEquipementCalc() {
...
// Fonction  : showCalendrierEquipementCalc
//              (Utilisation de la méthode AJAX)
//       c) Réponse de la SERVLET reçue
//              on recharge le calendrier après modification
function showCalendrierEquipementCalc() {
